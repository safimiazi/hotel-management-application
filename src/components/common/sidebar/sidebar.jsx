...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
  ...
  const location = useLocation();
  const [mouseHover, mouseHoverSet] = useState(false);
  function Onhover() {
  ...
  const { sideDashTextToggle } = useContext(AuthContext);
          ...
          <nav className="main-menu-container nav nav-pills flex-column sub-open">
            ...
            <Link
              ...
              className={` hover:text-black flex items-center ${
                sideDashTextToggle === false ? "ml-6" : "ml-8"
              }  gap-2 mb-1 mt-1  text-[18px] cursor-pointer text-gray-900 hover:text-gray-900 transition-colors duration-200 `}
            >
              <DashboardIcon className="text-gray-950" sx={{ width: "20px" }} />
              {sideDashTextToggle === false && window.innerWidth >= 992 ? (
                <span>Dashboard</span>
              ) : (
                <span className={`${mouseHover === true ? "block" : "hidden"}`}>
                  Dashboard
                </span>
              )}
              {window.innerWidth < 992 && <span>Dashboard</span>}
            </Link>
