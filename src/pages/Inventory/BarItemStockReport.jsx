...
const BarItemStockReport = () => {
  ...
  const [barItemStockData, barItemStockDataSet] = useState([]);
  ...
  useEffect(() => {
    const total = barItemStockData.reduce(
          ...
          <div className="print-content p-4">
            <TableContainer className="report-dom">
              <Table size="small" aria-label="a dense table">
                ...
                <TableBody>
                  {barItemStockData.map((stockItem, inx) => (
                    <>
                      <TableRow>
  ...
  const getItemStockReport = async () => {
    try {
      ...
      barItemStockDataSet(response.data.report);
      loadingSet(false);
    } catch (error) {
      Swal.fire({
        icon: "error",
...
export default BarItemStockReport;
