...
const SaleEntry = () => {
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/ItemSales/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
  ...
  const postItemSales = async () => {
    if (selectCustomer === null) {
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(`${API_URL}api/v1/ItemSales`, postData, {
            headers: authHeaders,
          });
        }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(`/sales/sale-invoice/${response.data.id}`);
            } else {
              resetForm();
            }
          });
