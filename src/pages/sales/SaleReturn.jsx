...
const SaleReturn = () => {
  ...
  const [salesReturnInvoiceTransactions, salesReturnInvoiceTransactionsSet] =
    useState([]);
  ...
  const handlePayment = () => {
    if (selectedAccount === null) {
      ...
      if (paymentIndex != null) {
        let copySaleInvoiceTransactions = [...salesReturnInvoiceTransactions];
        ...
        salesReturnInvoiceTransactionsSet(copySaleInvoiceTransactions);
        paymentIndexSet(null);
      } else {
        salesReturnInvoiceTransactionsSet([
          ...salesReturnInvoiceTransactions,
          transactionItem,
        ]);
        paymentIndexSet(null);
      }
  ...
  const handlePaymentDelete = (inx) => {
    const filteredItem = salesReturnInvoiceTransactions.filter(
      (item, index) => index != inx
    );
    salesReturnInvoiceTransactionsSet(filteredItem);
    paymentIndexSet(null);
  };

  useEffect(() => {
    const payTotalAmount = salesReturnInvoiceTransactions.reduce(
  ...
  useEffect(() => {
    ...
    if (pathSplitter(location.pathname, 2) != undefined) {
      getSaleReturnWithDetails();
      isSavingSet(true);
    }
  }, []);

  const getSaleReturnWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/itemSalesReturn/${pathSplitter(
          location.pathname,
          2
        )}`,
        {
  ...
  const handleEdit = (data) => {
    ...
    selectedItemSet();
    cartDataSet(
      data.itemSalesReturnDetails.map((itemSalesReturn) => ({
        name: itemSalesReturn.item.name,
        itemId: itemSalesReturn.item.id,
        qty: itemSalesReturn.qty,
        rate: itemSalesReturn.rate,
        amount: itemSalesReturn.amount,
      }))
    );
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/itemSalesReturn/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
  ...
  const postitemSalesReturnsReturn = async () => {
    if (selectCustomer === null) {
      ...
      const postData = {
        ...
        itemSalesReturnDetails: cartData.map((item, inx) => ({
          itemId: item.itemId,
          qty: item.qty,
          rate: item.rate,
          amount: item.amount,
        ...
        invoiceTransactions: salesReturnInvoiceTransactions.map(
          (transactionItem, inx) => ({
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(
            `${API_URL}api/v1/itemSalesReturn`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/itemSalesReturn/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(`/sales/sale-return-invoice/${response.data.id}`);
            } else {
              resetForm();
            }
          });
  ...
  return (
    <>
      <Paper className="m-3 p-3">
        ...
        <Stack direction="row" spacing={2}>
          <Button
            onClick={() => postitemSalesReturnsReturn()}
            variant="outlined"
        ...
        <TableContainer style={{ marginTop: "20px", width: "100%" }}>
          <Table size="small" aria-label="a dense table">
            ...
            <TableBody>
              ...
              {salesReturnInvoiceTransactions?.map((transactionItem, inx) => (
                <TableRow>
