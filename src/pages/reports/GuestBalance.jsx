...
const GuestBalance = () => {
  ...
  const filterType = [{ type: "All" }, { type: "By Guest" }];
  ...
  const [guestBalanceData, guestBalanceDataSet] = useState(null);
          ...
          <div className="print-content p-4">
            <TableContainer className="report-dom">
              <Table size="small" aria-label="a dense table">
                ...
                <TableBody>
                  {guestBalanceData.report.map((reportItem, inx) => (
                    <>
                      <TableRow>
                        ...
                        <TableCell style={{ textAlign: "right" }}>
                          {amountFormat(
                            parseFloat(reportItem.bookingBillAmount).toFixed(2)
                          )}
                        </TableCell>
                        <TableCell style={{ textAlign: "right" }}>
                          {amountFormat(
                  ...
                  <TableRow>
                    ...
                    <TableCell style={{ textAlign: "right" }}>
                      {amountFormat(
                        parseFloat(guestBalanceData.totalBalance).toFixed(2)
                      )}
                    </TableCell>
                  </TableRow>
                </TableBody>
  ...
  //handle report:
  const handleReport = () => {
    if (selectFilterType.type === "By Guest" && selectGuest === null) {
      Swal.fire({
  ...
  const getGuestBalance = async () => {
    try {
      ...
      guestBalanceDataSet(response.data);
    ...
    <div>
      <Paper className="m-3 p-3">
        ...
        >
          ...
          {selectFilterType != null && selectFilterType.type === "By Guest" && (
            <Grid item xs={12} md={2} sm={2}>
              <Autocomplete
                autoHighlight={true}
                openOnFocus={true}
