...
const PayrollBalance = () => {
  ...
  const [payrollBalanceData, payrollBalanceDataSet] = useState(null);
          ...
          <div className="print-content p-4">
            <TableContainer className="report-dom">
              <Table size="small" aria-label="a dense table">
                ...
                <TableBody>
                  {payrollBalanceData.report.map((reportItem, inx) => (
                    <>
                      <TableRow>
                  ...
                  <TableRow>
                    ...
                    <TableCell style={{ textAlign: "right" }}>
                      {amountFormat(
                        parseFloat(payrollBalanceData.totalBalance).toFixed(2)
                      )}
                    </TableCell>
                  </TableRow>
                </TableBody>
  ...
  //handle report:
  const handleReport = () => {
    if (selectFilterType.type === "By Employee" && selectEmployee === null) {
      Swal.fire({
    ...
    getPayrollBalanceData();
  };

  const getPayrollBalanceData = async () => {
    try {
      loadingSet(true);

      const response = await axios.get(
        `${API_URL}api/v1/Report/get-payroll-balance`,
        {
          headers: authHeaders,
          params: {
            EmployeeId:
      ...
      payrollBalanceDataSet(response.data);
