...
const SupplierLedger = () => {
  ...
  const [supplierLedgerData, supplierLedgerDataSet] = useState(null);
              ...
              <Table size="small" aria-label="a dense table">
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell style={{ textAlign: "right" }}>
                      {amountFormat(
                        parseFloat(supplierLedgerData.openingBalance).toFixed(2)
                      )}
                    </TableCell>
                  </TableRow>
                  {supplierLedgerData.ledger.map((ledgerItem, inx) => (
                    <>
                      <TableRow>
                  ...
                  <TableRow>
                    ...
                    <TableCell style={{ textAlign: "right" }}>
                      {amountFormat(
                        parseFloat(supplierLedgerData.closingBalance).toFixed(2)
                      )}
                    </TableCell>
                  </TableRow>
                </TableBody>
  ...
  const getCustomerLedger = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}api/v1/Report/get-supplier-ledger`,
        {
          headers: authHeaders,
          params: {
            SupplierId: selectSupplier.id ? selectSupplier.id : null,
            FromDate: selectedFromDate.format(dateFormat),
            ToDate: selectedToDate.format(dateFormat),
          },
        }
      );
      supplierLedgerDataSet(response.data);
