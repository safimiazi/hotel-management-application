...
const PremiumServices = () => {
  ...
  //for get primiumServiceNames
  const [primiumServiceNames, primiumServiceNamesSet] = useState([]);
  //for add to cart
  const [selectedprimiumServiceName, selectedprimiumServiceNameset] =
    useState(null);
  ...
  useEffect(() => {
    const amount = parseFloat(qty) * parseFloat(charge);
    amountSet(amount);
  ...
  const addToCart = () => {
    if (selectedprimiumServiceName === null) {
      Swal.fire({
      ...
      const cartItem = {
        name: selectedprimiumServiceName.name,
        serviceNameId: selectedprimiumServiceName.id,
        qty: qty,
        charge: charge,
        amount: amount,
      };
      ...
      selectedprimiumServiceNameset(null);
      qtySet(0);
      chargeSet(0);
      amountSet(0);
    }
  ...
  const cartUpdate = (row, inx) => {
    selectedprimiumServiceNameset({ name: row.name, id: row.serviceNameId });
  ...
  const getPremiumServiceWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/PremiumService/${pathSplitter(location.pathname, 2)}`,
        {
          headers: authHeaders,
        }
      );
  ...
  const handleEdit = (data) => {
    ...
    cartDataSet(
      data.serviceDetails.map((serviceItem) => ({
        name: serviceItem.serviceName.name,
        serviceNameId: serviceItem.serviceName.id,
        qty: serviceItem.qty,
        charge: serviceItem.charge,
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/PremiumService/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
  ...
  const postPremiumService = async () => {
    if (selectcustomer === null) {
      ...
      const postData = {
        ...
        serviceDetail: cartData.map((item, inx) => ({
      ...
      try {
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/PremiumService/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(
                `/premium-service/premium-service-invoice/${response.data.id}`
              );
            } else {
              resetForm();
            }
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              resetForm();
              if (action === "put") {
                navigate(`/premium-service/service-entry`);
              }
            }
          });
        }
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      ...
      if (e.target === customerRef.current) {
    ...
    <>
      <Paper className="m-3 p-3">
        ...
        >
          ...
          <Grid item xs={12} sm={3} className="plus-link-div">
            <Link to="/premium-service/customer-entry" className="plus-link">
              +
            </Link>
            <Autocomplete
              autoHighlight={true}
        ...
        >
          <Grid item xs={12} sm={3} className="plus-link-div">
            <Link
              to="/premium-service/service-name-entry"
              className="plus-link"
            >
              +
            </Link>
            <Autocomplete
              ...
              size="small"
              id="combo-box-gender"
              options={primiumServiceNames}
              value={selectedprimiumServiceName}
              onKeyDown={handleKeyPress}
              onChange={(e, obj) => {
                selectedprimiumServiceNameset(obj);
                if (obj != null) {
                  chargeSet(obj.charge);
                } else {
                  chargeSet(0);
                }
              }}
          ...
          <Grid item xs={12} sm={3}>
            <TextField
              ...
              onChange={(e) => {
                ...
                chargeSet(parseFloat(e.target.value));
              }}
              onBlur={(e) => {
                if (e.target.value === "" || parseFloat(e.target.value) < 0) {
                  chargeSet(0);
                  ...
                  {cartData.map((row, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell align="right">
                        {amountFormat(parseFloat(row.charge))}
                      </TableCell>
                      <TableCell align="right">
      ...
      <Modal
        ...
        style={{
          ...
          alignprimiumServiceNames: "center",
