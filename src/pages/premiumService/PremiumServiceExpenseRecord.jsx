...
const PremiumServiceExpenseRecord = () => {
  ...
  const [selecteSupplier, selecteSupplierSet] = useState(null);
  ...
  useEffect(() => {
    const total = serviceExpenseData
        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/PremiumServiceExpense/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
            ...
            <div className="print-content p-4">
              <TableContainer className="report-dom">
                <Table size="small" aria-label="a dense table">
                  ...
                  <TableBody>
                    {serviceExpenseData.map((expenseItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "left" }}>
                            {expenseItem.invoiceNo}
                          </TableCell>
                          <TableCell style={{ textAlign: "left" }}>
                            {expenseItem.supplier.name}
                          </TableCell>
                          <TableCell style={{ textAlign: "left" }}>
                            {dayjs(expenseItem.createdDate).format(dateFormat)}
                          </TableCell>
                          <TableCell style={{ textAlign: "left" }}>
                            {expenseItem.creator.userName}
                          </TableCell>

                          <TableCell style={{ textAlign: "left" }}>
                            {expenseItem.expenseDetails[0].expenseName.name}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                expenseItem.expenseDetails[0].cost
                              ).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {expenseItem.expenseDetails[0].qty}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                expenseItem.expenseDetails[0].amount
                              ).toFixed(2)
                            )}
                          </TableCell>
                          ...
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.totalAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell
                            className="print-no"
                          ...
                          >
                            <div className="flex items-center gap-3 justify-around">
                              <Link
                                to={{
                                  pathname: `/premium-service/premium-service-expense-invoice/${expenseItem.id}`,
                                }}
                              >
                                <ReceiptIcon
                                  style={{
                              ...
                              <Link
                                to={{
                                  pathname: `/premium-service/premium-service-expense/${expenseItem.id}`,
                                }}
                              >
                                <EditNoteIcon
                                  style={{
                        ...
                        {expenseItem.expenseDetails.slice(1).map((detail) => (
                          <TableRow>
                            <TableCell colSpan={5}></TableCell>

                            <TableCell style={{ textAlign: "left" }}>
                              {detail.expenseName.name}
                            </TableCell>

                            <TableCell style={{ textAlign: "right" }}>
                              {amountFormat(parseFloat(detail.cost).toFixed(2))}
                            </TableCell>
                            <TableCell style={{ textAlign: "right" }}>
                              {detail.qty}
                            </TableCell>
                    ...
                    {serviceExpenseData.map((expenseItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "left" }}>
                            {expenseItem.updater.userName}
                          </TableCell>

                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.discountAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.dueAmount).toFixed(2)
                            )}
                          </TableCell>

                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.othersCharge).toFixed(2)
                            )}
                          </TableCell>

                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.taxAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.paidAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(expenseItem.subTotal).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
  ...
  const getPremiumserviceExpenseData = async () => {
    try {
      loadingSet(true);
      let url = ``;
      if (selectedRecordType.type == "With Details") {
        url = `PremiumServiceExpense`;
      }
      if (selectedRecordType.type == "Without Details") {
        url = `PremiumServiceExpense/get-service-expense-record`;
      }
      const response = await axios.get(`${API_URL}api/v1/${url}`, {
        headers: authHeaders,
        params: {
          SupplierId: selecteSupplier != null ? selecteSupplier.id : null,
  ...
  return (
    <div>
      <Paper className="m-3 p-3">
        ...
        >
          PREMIUM SERVICE EXPENSE RECORD
        </Typography>
...
export default PremiumServiceExpenseRecord;
