...
const PremiumServiceRecord = () => {
  ...
  const [selecteCustomer, selecteCustomerset] = useState(null);
        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/PremiumService/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
            ...
            <div className="print-content p-4">
              <TableContainer className="report-dom">
                <Table size="small" aria-label="a dense table">
                  ...
                  <TableBody>
                    {servicedata.map((serviceItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "left" }}>
                            {serviceItem.invoiceNo}
                          </TableCell>
                          <TableCell style={{ textAlign: "left" }}>
                            {serviceItem.customer.name}
                          </TableCell>
                          <TableCell style={{ textAlign: "left" }}>
                            {dayjs(serviceItem.createdDate).format(dateFormat)}
                          </TableCell>
                          <TableCell style={{ textAlign: "left" }}>
                            {serviceItem.creator.userName}
                          </TableCell>

                          <TableCell style={{ textAlign: "left" }}>
                            {serviceItem.serviceDetails[0].serviceName.name}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                serviceItem.serviceDetails[0].charge
                              ).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {serviceItem.serviceDetails[0].qty}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                serviceItem.serviceDetails[0].amount
                              ).toFixed(2)
                            )}
                          </TableCell>
                          ...
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.totalAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell
                            className="print-no"
                          ...
                          >
                            <div className="flex items-center gap-3 justify-around">
                              <Link
                                to={{
                                  pathname: `/premium-service/premium-service-invoice/${serviceItem.id}`,
                                }}
                              >
                                <ReceiptIcon
                                  style={{
                              ...
                              <Link
                                to={{
                                  pathname: `/premium-service/premium-service/${serviceItem.id}`,
                                }}
                              >
                                <EditNoteIcon
                                  style={{
                              ...
                              <DeleteForeverIcon
                                ...
                                onClick={() => handleDelete(serviceItem)}
                              />
                            </div>
                          </TableCell>
                        </TableRow>

                        {serviceItem.serviceDetails.slice(1).map((detail) => (
                          <TableRow>
                            ...
                            <TableCell style={{ textAlign: "right" }}>
                              {amountFormat(
                                parseFloat(detail.charge).toFixed(2)
                              )}
                            </TableCell>
                            <TableCell style={{ textAlign: "right" }}>
                              {detail.qty}
                    ...
                    {servicedata.map((serviceItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "left" }}>
                            {serviceItem.updater.userName}
                          </TableCell>

                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.discountAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.dueAmount).toFixed(2)
                            )}
                          </TableCell>

                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.othersCharge).toFixed(2)
                            )}
                          </TableCell>

                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.taxAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.paidAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(serviceItem.subTotal).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
  ...
  const getPremiumServiceData = async () => {
    try {
      loadingSet(true);
      let url = ``;
      ...
      if (selectedRecordType.type == "Without Details") {
        url = `PremiumService/get-service-record`;
      }
      const response = await axios.get(`${API_URL}api/v1/${url}`, {
        headers: authHeaders,
        params: {
          CustomerId: selecteCustomer != null ? selecteCustomer.id : null,
...
export default PremiumServiceRecord;
