...
const HouseKeepingItemPurchaseReturnRecord = () => {
        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/HouseKeepingItemPurchaseReturn/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
                    ...
                    {purchaseData.map((purchaseItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "left" }}>
                            {
                              purchaseItem.itemPurchaseReturnDetails[0].item
                                .name
                            }
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                purchaseItem.itemPurchaseReturnDetails[0].rate
                              ).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {purchaseItem.itemPurchaseReturnDetails[0].qty}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                purchaseItem.itemPurchaseReturnDetails[0].amount
                              ).toFixed(2)
                            )}
                          </TableCell>
                          ...
                          >
                            <div className="flex items-center gap-3 justify-around">
                              <Link
                                to={{
                                  pathname: `/house-keeping/house-keeping-purchase-return-invoice/${purchaseItem.id}`,
                                }}
                              >
                                <ReceiptIcon
                                  style={{
                              ...
                              <Link
                                to={{
                                  pathname: `/house-keeping/house-keeping-item-purchase-return/${purchaseItem.id}`,
                                }}
                              >
                                <EditNoteIcon
                                  style={{
                        ...
                        {purchaseItem.itemPurchaseReturnDetails
                          .slice(1)
                          .map((detail) => (
                            <TableRow>
  ...
  const getPurchase = async () => {
    try {
      ...
      if (selectedRecordType.type == "With Details") {
        url = `HouseKeepingItemPurchaseReturn`;
      }
      if (selectedRecordType.type == "Without Details") {
        url = `HouseKeepingItemPurchaseReturn/get-item-purchase-record`;
      }
  ...
  return (
    <div>
      <Paper className="m-3 p-3">
        ...
        >
          HOUSE KEEPING PURCHASE RETURN RECORD
        </Typography>
...
export default HouseKeepingItemPurchaseReturnRecord;
