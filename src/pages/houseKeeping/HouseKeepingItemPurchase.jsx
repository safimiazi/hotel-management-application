...
const HouseKeepingItemPurchase = () => {
  ...
  const getPurchaseWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/HouseKeepingItemPurchase/${pathSplitter(
          location.pathname,
          2
        )}`,
        {
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/HouseKeepingItemPurchase/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(
            `${API_URL}api/v1/HouseKeepingItemPurchase`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/HouseKeepingItemPurchase/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(
                `/house-keeping/house-keeping-purchase-invoice/${response.data.id}`
              );
            } else {
              resetForm();
            }
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              resetForm();
              if (action === "put") {
                navigate(`/house-keeping/hk-item-purchase`);
              }
            }
          });
  ...
  return (
    <>
      <Paper className="m-3 p-3">
        ...
        >
          HOUSE KEEPING ITEM PURCHASE ENTRY
        </Typography>
        <Grid
          container
          spacing={3}
        ...
        >
          ...
          <Grid item xs={12} sm={3} className="plus-link-div">
            <Link to="/house-keeping-purchase/supplier" className="plus-link">
              +
            </Link>
            <Autocomplete
              autoHighlight={true}
...
export default HouseKeepingItemPurchase;
