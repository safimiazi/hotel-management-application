...
const HouseKeepingItemPurchaseReturn = () => {
  ...
  const getPurchaseWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/HouseKeepingItemPurchaseReturn/${pathSplitter(
          location.pathname,
          2
        )}`,
        {
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/HouseKeepingItemPurchaseReturn/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
  ...
  const postItemPurchaseReturn = async () => {
    if (selectSupplier === null) {
      ...
      const postData = {
        ...
        purchaseReturnInvoiceTransactions: purchaseInvoiceTransactions.map(
          (transactionItem, inx) => ({
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(
            `${API_URL}api/v1/HouseKeepingItemPurchaseReturn`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/HouseKeepingItemPurchaseReturn/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(
                `/house-keeping/house-keeping-purchase-return-invoice/${response.data.id}`
              );
            } else {
              resetForm();
            }
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              resetForm();
              if (action === "put") {
                navigate(`/house-keeping/hk-purchase-return`);
              }
            }
          });
        }
  ...
  return (
    <>
      <Paper className="m-3 p-3">
        ...
        >
          HOUSE KEEPING ITEM PURCHASE RETURN ENTRY
        </Typography>
        <Grid
          container
          spacing={3}
...
export default HouseKeepingItemPurchaseReturn;
