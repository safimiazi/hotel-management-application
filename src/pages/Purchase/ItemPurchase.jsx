...
const ItemPurchase = () => {
  ...
  const getPurchaseWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/ItemPurchase/${pathSplitter(location.pathname, 2)}`,
        {
          headers: authHeaders,
        }
      );
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/ItemPurchase/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
      ...
      try {
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/ItemPurchase/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(`/purchase/item-purchase-invoice/${response.data.id}`);
            } else {
              resetForm();
            }
          });
