        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/BarItemSale/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
                    ...
                    {saleData.map((saleItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "left" }}>
                            {dayjs(saleItem.creator.userName).format(
                              dateFormat
                            )}
                          </TableCell>
                          ...
                          >
                            <div className="flex items-center gap-3 justify-around">
                              <Link
                                to={{
                                  pathname: `/bar/bar-item-sale-invoice/${saleItem.id}`,
                                }}
                              >
                                <ReceiptIcon
                                  style={{
                              ...
                              <Link
                                to={{
                                  pathname: `/bar/bar-item-sale/${saleItem.id}`,
                                }}
                              >
                                <EditNoteIcon
                                  style={{
  ...
  const getItemSales = async () => {
    try {
      ...
      if (selectedRecordType.type == "Without Details") {
        url = `BarItemSale/get-item-sales-record`;
      }
...
export default BarItemSaleRecord;
