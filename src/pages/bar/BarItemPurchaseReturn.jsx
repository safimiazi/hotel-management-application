...
const BarItemPurchaseReturn = () => {
  ...
  const handlePayment = () => {
    if (selectedAccount === null) {
      ...
      if (paymentIndex != null) {
        let copypurchaseReturnInvoiceTransactions = [
          ...purchaseReturnInvoiceTransactions,
        ];
        copypurchaseReturnInvoiceTransactions[paymentIndex] = {
          name: selectedAccount.name,
          payAccountId: selectedAccount.id,
          amount: paymentAmountValue,
        };
        ...
        purchaseReturnInvoiceTransactionsSet(
          copypurchaseReturnInvoiceTransactions
        );
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/BarItemPurchaseReturn/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(
            `${API_URL}api/v1/BarItemPurchaseReturn`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/BarItemPurchaseReturn/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(
                `/bar/bar-item-purchase-return-invoice/${response.data.id}`
              );
            } else {
              resetForm();
            }
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              resetForm();
              if (action === "put") {
                navigate(`/bar/bar-item-purchase-return`);
              }
            }
          });
        }
  ...
  return (
    <>
      <Paper className="m-3 p-3">
        ...
        >
          BAR ITEM PURCHASE RETURN ENTRY
        </Typography>
        <Grid
          container
          spacing={3}
...
export default BarItemPurchaseReturn;
