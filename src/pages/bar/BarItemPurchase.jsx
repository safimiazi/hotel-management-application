...
const BarItemPurchase = () => {
  ...
  const getPurchaseWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/BarItemPurchase/${pathSplitter(
          location.pathname,
          2
        )}`,
        {
  ...
  const handleEdit = (data) => {
    ...
    selectedItemSet();
    cartDataSet(
      data.itemPurchaseDetails.map((itemPurchase) => ({
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/BarItemPurchase/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
  ...
  const postItemPurchase = async () => {
    if (selectSupplier === null) {
      ...
      const postData = {
        ...
        itemPurchaseDetails: cartData.map((item, inx) => ({
          itemId: item.itemId,
          qty: item.qty,
          rate: item.rate,
          amount: item.amount,
        ...
        purchaseInvoiceTransactions: purchaseInvoiceTransactions.map(
          (transactionItem, inx) => ({
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(
            `${API_URL}api/v1/BarItemPurchase`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/BarItemPurchase/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              navigate(`/bar/bar-item-purchase-invoice/${response.data.id}`);
            } else {
              resetForm();
            }
          });
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              resetForm();
              if (action === "put") {
                navigate(`/bar/bar-item-purchase`);
              }
            }
          });
        }
  ...
  const suppllierRef = useRef(null);
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      ...
      if (e.target === invoiceRef.current) {
        suppllierRef.current.focus();
      } else if (e.target === suppllierRef.current) {
      ...
      <Paper className="m-3 p-3">
        ...
        >
          ...
          <Grid item xs={12} sm={3} className="plus-link-div">
            <Link to="/bar/supplier" className="plus-link">
              +
            </Link>
...
export default BarItemPurchase;
