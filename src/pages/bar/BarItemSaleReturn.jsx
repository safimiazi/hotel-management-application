...
const BarItemSaleReturn = () => {
  ...
  useEffect(() => {
    ...
    const totalAmount = cartData.reduce(
      (prev, cartItem) => parseFloat(prev) + parseFloat(cartItem.amount),
      0
    );
  ...
  const getSaleWithDetails = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/BarItemSaleReturn/${pathSplitter(
          location.pathname,
          2
        )}`,
        {
  ...
  const handleEdit = (data) => {
    ...
    selectedItemSet();
    cartDataSet(
      data.itemSalesReturnDetails.map((itemSale) => ({
  ...
  const getInvoiceCode = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/v1/BarItemSaleReturn/get-invoice-code`,
        {
          headers: authHeaders,
        }
      );
  ...
  const postItemSalesReturn = async () => {
      ...
      try {
        ...
        if (action == "post") {
          response = await axios.post(
            `${API_URL}api/v1/BarItemSaleReturn`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action == "put") {
          response = await axios.put(
            `${API_URL}api/v1/BarItemSaleReturn/${id}`,
            postData,
            {
              headers: authHeaders,
            }
        ...
        if (action === "post") {
          Swal.fire({
            title: `Sale Return Successfully. Do you want to view the invoice?`,
            ...
            if (result.isConfirmed) {
              navigate(`/bar/bar-item-sale-return-invoice/${response.data.id}`);
            } else {
              resetForm();
            }
          });
          ...
          Swal.fire({
            title: `Sale Return update Successfully. Do you want to view the invoice?`,
            ...
            if (result.isConfirmed) {
              ...
              if (action === "put") {
                navigate(`/bar/bar-item-sale-return`);
              }
            }
          });
        }
...
export default BarItemSaleReturn;
