...
const BarItemPurchaseRecord = () => {
        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/BarItemPurchase/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
                      ...
                      <TableRow>
                        ...
                        >
                          <div className="flex items-center gap-3 justify-around">
                            <Link
                              to={{
                                pathname: `/bar/bar-item-purchase-invoice/${purchaseItem.id}`,
                              }}
                            >
                              <ReceiptIcon
                                style={{
                            ...
                            <Link
                              to={{
                                pathname: `/bar/bar-item-purchase/${purchaseItem.id}`,
                              }}
                            >
                              <EditNoteIcon
                                style={{
                    ...
                    {purchaseData.map((purchaseItem, inx) => (
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(purchaseItem.vatAmount).toFixed(2)
                            )}
                          </TableCell>
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
  ...
  const handleReport = () => {
    if (selectedRecordType === null) {
      Swal.fire({
        title: "Please Select Record Type.",
        icon: "warning",
      });
      return false;
    }
  ...
  const getPurchase = async () => {
    try {
      ...
      if (selectedRecordType.type == "Without Details") {
        url = `BarItemPurchase/get-item-purchase-record`;
      }
...
export default BarItemPurchaseRecord;
