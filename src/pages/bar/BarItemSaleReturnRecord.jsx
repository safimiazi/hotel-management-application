        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/BarItemSaleReturn/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
                        ...
                        <TableRow>
                          ...
                          >
                            <div className="flex items-center gap-3 justify-around">
                              <Link
                                to={{
                                  pathname: `/bar/bar-item-sale-return-invoice/${saleReturnItem.id}`,
                                }}
                              >
                                <ReceiptIcon
                                  style={{
                              ...
                              <Link
                                to={{
                                  pathname: `/bar/bar-item-sale-return/${saleReturnItem.id}`,
                                }}
                              >
                                <EditNoteIcon
                                  style={{
  ...
  const getItemSalesReturn = async () => {
    try {
      ...
      if (selectedRecordType.type == "With Details") {
        url = `BarItemSaleReturn`;
      }
      if (selectedRecordType.type == "Without Details") {
        url = `BarItemSaleReturn/get-item-sales-return-record`;
      }
...
export default BarItemSaleReturnRecord;
