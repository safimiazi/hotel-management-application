...
const BarItemPurchaseReturnRecord = () => {
        ...
        try {
          if (res.isConfirmed) {
            let response = await axios.delete(
              `${API_URL}api/v1/BarItemPurchaseReturn/${row.id}`,
              { headers: authHeaders },
              { id: row.id }
            );
            Swal.fire({
                      ...
                      <>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "right" }}>
                            {amountFormat(
                              parseFloat(
                                purchaseItem?.itemPurchaseReturnDetails[0]
                                  .amount
                              ).toFixed(2)
                            )}
                          </TableCell>
                          ...
                          >
                            <div className="flex items-center gap-3 justify-around">
                              <Link
                                to={{
                                  pathname: `/bar/bar-item-purchase-return-invoice/${purchaseItem.id}`,
                                }}
                              >
                                <ReceiptIcon
                                  style={{
                              ...
                              <Link
                                to={{
                                  pathname: `/bar/bar-item-purchase-return/${purchaseItem.id}`,
                                }}
                              >
                                <EditNoteIcon
                                  style={{
  ...
  const getPurchase = async () => {
    try {
      ...
      if (selectedRecordType.type == "With Details") {
        url = `BarItemPurchaseReturn`;
      }
      if (selectedRecordType.type == "Without Details") {
        url = `BarItemPurchaseReturn/get-item-purchase-return-record`;
      }
...
export default BarItemPurchaseReturnRecord;
